...
// This code is adapetd from:
// Yet Another Hierarchical State Machine by Stefan Heinzmann
// Overload issue 64 december 2004
// https://accu.org/index.php/journals/252

/* The key observation is that the machine can only be in a leaf state at any given time.
 * The composite states are only traversed, never final.
 * Only the leaf states are ever instantiated. */
...
struct LeafState : B {
    ...
    static void init(H& h) { h.next(obj); }             // announce itself to the host class as the new state
    static void entry(H&) {}
    static void exit(H&) {}
...
// Transitions : is D derived from B (directly or indirectly)?, from Herb Sutter's GotW #71 :
...
public:
    enum { Res = sizeof(Test(static_cast<D*>(0))) == sizeof(Yes) ? 1 : 0 };         // PAS COMPRIS!
};
